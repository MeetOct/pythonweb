...
def init_jinja2(app, **kw):
	...
	env = Environment(loader=FileSystemLoader([path,os.path.join(path,'test')]), **options)
	#从参数取filter字段
...
def init(loop):
    ...
    handles.add_routes(app,'routes')
    handles.add_static(app) 
